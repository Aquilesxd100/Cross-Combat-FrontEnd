...
async function gerarCardsAnimes(qntCardsAGerar : number, tipoCard : string,  nomesCardsRegistrados : Array<string>) {
    const infosAPI = await fetch(`https://cross-combat-api.onrender.com/gerarPersonagensAnimes/${qntCardsAGerar}`, {
        method: "POST",
        headers: {
    ...
    const cardsGerados : Array<CardType> = infosAPI.map((cardBruto : any) => {
        ...
        return({
            ...
            trunfo: trunfoStatus,
            universo: "Animes",
            nome: cardBruto.attributes.canonicalName,
            imagem: cardBruto.attributes.image.original,
...
export default gerarCardsAnimes;