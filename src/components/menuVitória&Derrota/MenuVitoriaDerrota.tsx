    ...
    const confirmButtonHandler = () => {
        if (modalVitoriaActive && !cardsPreLoadingState) {
            ...
            setTimeout(() => {
                ...
                dispatch(setPendingCristalAnimation(false));
            }, 200);
    ...
    return (
        <div ref={fundoTela} className="fixed flex items-center justify-center top-0 left-0 h-full w-full bg-black/50 z-10 backdrop-blur-[1.5px]" style={modalDisplay}>
            <div className="relative flex flex-col items-center justify-center w-[calc(30vw+140px)] h-[calc(24.6vw+140px)] bg-100% top-[5%]" style={{backgroundImage:`url(${IMGModalMenuMaior})`}}>
                <img src={modalLogo} className="absolute w-[75%] h-[85%] top-[-31%] sombra-padrao-img off-user-selection" />
                <button ref={botaoConfirmar} className="min-w-[28%] w-[59%] h-[21%] bg-100% bg-no-repeat my-2 text-[3vw] font-bold brightness-[0.85] hover:brightness-110 top-[60%] absolute" style={{backgroundImage: `url(${botaoPadrao})`}} onClick={(() => { confirmButtonHandler(); })}><h3 className="gradiente-laranja off-user-selection" ref={textoConfirmar}>TELA INICIAL</h3></button>
            </div>
        </div>
    );
};
