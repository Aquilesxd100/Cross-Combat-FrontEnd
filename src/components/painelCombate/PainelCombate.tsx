...
import { setInfosModal, setMenuModal } from "../../redux/slices/modalSlice";
...
function PainelCombate() {
    ...
    return (
        <div className="h-[42vh] w-[13%] bg-100% fixed top-[30vh] left-0 flex flex-col items-center justify-center pr-5" style={{backgroundImage:`url(${IMGPainel})`}}>
            <button onClick={(() => { dispatch(setMenuModal(true)); dispatch(activateEffect('botaoPadrao')) })} className="z-10 bg-100% w-[72%] h-[34%] brightness-[0.85] hover:brightness-105 mb-2" style={{backgroundImage:`url(${IMGBotaoMenu})`}} />
            <button onClick={(() => { dispatch(setInfosModal(true)); dispatch(activateEffect('botaoPadrao')) })} className="z-10 bg-100% w-[72%] h-[34%] brightness-[0.85] hover:brightness-105" style={{backgroundImage:`url(${IMGBotaoInfo})`}} />
        </div>
    )
}
