    ...
    return (
        <div ref={divModalMenu} className="fixed flex items-center justify-center top-0 left-0 h-full w-full bg-black/50 z-10 backdrop-blur-[1.5px]" style={modalDisplay}>
            <div className="relative flex flex-col items-center justify-center w-[calc(40vw+100px)] h-[calc(26vw+50px)] bg-100%" style={{backgroundImage:`url(${IMGModalMenu})`}}>
                ...
                <button className="min-w-[25%] w-[54%] h-[25%] bg-100% bg-no-repeat my-2 text-[3vw] font-bold brightness-[0.85] hover:brightness-110" style={{backgroundImage: `url(${botaoPadrao})`}} onClick={(() => { telaInicialHandler(); dispatch(activateEffect('botaoPadrao')); })}><h3 className="gradiente-laranja off-user-selection">TELA INICIAL</h3></button>
                <button className="min-w-[25%] w-[54%] h-[25%] bg-100% bg-no-repeat my-2 text-[3vw] font-bold brightness-[0.85] hover:brightness-110" style={{backgroundImage: `url(${botaoPadrao})`}} onClick={(() => { saveGameHandler() })} ref={botaoSalvar}><h3 className="gradiente-laranja off-user-selection">SALVAR</h3></button>
            </div>
        </div>
    );
};
